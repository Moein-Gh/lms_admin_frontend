...
import type { LoanFilters } from "./loan-filter-config";

type Props = {
  data: PaginatedResponseDto<Loan> | null;
  isLoading: boolean;
  error: unknown;
  filterConfig: EntityFilterConfig<LoanFilters>;
  filters: LoanFilters;
  onFiltersChange: (filters: LoanFilters) => void;
  onReset: () => void;
};

export function LoansTable({ data, isLoading, error, filterConfig, filters, onFiltersChange, onReset }: Props) {
