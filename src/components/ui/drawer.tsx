...
function Drawer({ open, ...props }: React.ComponentProps<typeof DrawerPrimitive.Root>) {
  React.useEffect(() => {
    try {
      if (open) {
...
function DrawerOverlay({ className, ...props }: React.ComponentProps<typeof DrawerPrimitive.Overlay>) {
  return (
    <DrawerPrimitive.Overlay
      ...
      className={cn(
        "data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 fixed inset-0 z-50 bg-black/40 backdrop-blur-lg",
        className
      )}
      {...props}
    />
  ...
  return (
    <DrawerPortal data-slot="drawer-portal">
      ...
      <DrawerPrimitive.Content
        ...
        className={cn(
          "group/drawer-content bg-background fixed z-50 flex h-auto flex-col max-h-[96dvh]",
...
function DrawerHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex flex-col gap-0.5 p-4 group-data-[vaul-drawer-direction=bottom]/drawer-content:text-center group-data-[vaul-drawer-direction=top]/drawer-content:text-center md:gap-1.5 md:text-end",
        className
      )}
      {...props}
    />
...
// New component to handle scrollable content area
...
export {
  ...
  DrawerContent,
  DrawerHeader,
  DrawerBody, // Export the new component
  DrawerFooter,
  DrawerTitle,
  DrawerDescription
};
