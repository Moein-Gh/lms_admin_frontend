...
export async function getFinancialSummary(params: FinancialSummaryParams): Promise<FinancialSummary> {
  const response = await api.get<FinancialSummary>(`/admin/report/dashboard/summary`, { params });
  return response.data;
}
export async function getEntitiesSummary(): Promise<EntitiesSummary> {
  const response = await api.get<EntitiesSummary>(`/admin/report/dashboard/entities`);
  return response.data;
}
...
export async function getInstallmentProjection(
  ...
  const response = await api.get<InstallmentProjection>(`/admin/report/projections/installments`, {
    params
  });
  return response.data;
}
