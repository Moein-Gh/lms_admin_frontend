...
export async function listAccounts(params?: ListAccountsParams): Promise<PaginatedResponseDto<Account>> {
  const response = await api.get<PaginatedResponseDto<Account>>(`/admin/accounts/`, {
    params
  });
...
export async function getAccountById(accountId: string): Promise<Account> {
  const response = await api.get<Account>(`/admin/accounts/${accountId}`);
  return response.data;
}

export async function createAccount(data: CreateAccountRequest): Promise<Account> {
  const response = await api.post<Account>(`/admin/accounts/`, data);
  return response.data;
}
export async function buyOutAccount(accountId: string): Promise<Account> {
  const response = await api.post<Account>(`/admin/accounts/${accountId}/buy-out`);
  return response.data;
}

export async function updateAccount(accountId: string, data: UpdateAccountRequest): Promise<Account> {
  const response = await api.patch<Account>(`/admin/accounts/${accountId}`, data);
  return response.data;
}
...
export async function activateAccount(accountId: string): Promise<Account> {
  const response = await api.patch<Account>(`/admin/accounts/${accountId}/activate`);
  return response.data;
}
