...
export async function getMessageTemplates(
  ...
  const response = await api.get<PaginatedResponseDto<MessageTemplate>>("/message-templates", { params });
  return response.data;
}
...
export async function restoreMessageTemplate(id: string): Promise<MessageTemplate> {
  const response = await api.post<MessageTemplate>(`/message-templates/${id}/restore`);
  return response.data;
}
