...
export async function getMessages(params?: MessageQueryParams): Promise<PaginatedResponseDto<Message>> {
  const response = await api.get<PaginatedResponseDto<Message>>("/messages", {
    params
  });
  return response.data;
}
...
export async function updateRecipientStatus(
  ...
  const response = await api.patch<MessageRecipient>(`/messages/recipients/${recipientId}/status`, data);
  return response.data;
}
